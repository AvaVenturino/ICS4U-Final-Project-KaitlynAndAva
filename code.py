...
def game_scene():
    ...
    alien_object = Alien(image_bank, 1, 74, 56) # change numbers to match bmp file ***
    ...
    for alien_number in range(constants.TOTAL_NUMBER_OF_LEFT_ALIENS):
        a_single_aliens = stage.Sprite(image_bank, 4, constants.OFF_SCREEN_X, constants.OFF_SCREEN_Y)
    ...
    for laser_number in range(constants.TOTAL_NUMBER_OF_LASERS):
        ...
        lasers_objects.add_to_list(a_laser_object)
    ...
    while True:
        ...
        for laser_counter, laser in enumerate(lasers_objects.lasers):
        ...
        if a_button == constants.button_state["button_just_pressed"]:
            lasers_objects.fire_laser(ship_object.x_pos, ship_object.y_pos, -1)

        if b_button == constants.button_state["button_just_pressed"]:
            lasers_objects.fire_laser(ship_object.x_pos, ship_object.y_pos, 1)
        
        lasers_objects.velocity(constants.LASER_SPEED)

        alien_objects.velocity(constants.ALIEN_SPEED)

        if lasers_objects.is_laser_off_screen():
            score -= 1
            if score < 0:
                score = 0
            score_text.clear()
        ...
        if alien_objects.is_laser_colliding(lasers_objects):
            # sound.stop()
        ...
        if alien_objects.is_ship_colliding(ship_object):
            # sound.stop()
            sound.play(boom_sound)
            time.sleep(2.0)
